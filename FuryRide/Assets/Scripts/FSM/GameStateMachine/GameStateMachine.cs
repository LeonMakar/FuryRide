...
public class GameStateMachine : StateManager<GameStateMachine.GameStates>
{
    ...
    [Inject]
    private void Cunstruct(EventBus eventBus, EnemyPool enemyPool, SaveService saveAndLoad, UIMoneyShower uIMoneyShower, BonusePool bonusePool)
    {
        GameIsActiveState gameIsActiveState = new GameIsActiveState(GameStates.GameIsActive, _gamePlayCanvas, _enemySpawner,
            _playerGameObject, eventBus, _menuZombie, enemyPool, uIMoneyShower, _bonusSpawner, bonusePool, _musicAudio, saveAndLoad);
